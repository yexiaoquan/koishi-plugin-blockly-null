...
let isTypeManagerRegistered = false
const originalInitMethods = new Map<string, Function>()

export function registerTypeManager(workspace: Workspace) {
  if (isTypeManagerRegistered) {
    workspace.typings = new ReactiveBindingSet<ReactiveValue<any>>()
    return
  }

  isTypeManagerRegistered = true
  workspace.typings = new ReactiveBindingSet<ReactiveValue<any>>()

  Object.keys(Blockly.Blocks).forEach(k => {
    if (k.startsWith('type_')) return

    if (!originalInitMethods.has(k)) {
      originalInitMethods.set(k, Blockly.Blocks[k].init)
    }

    const _init = originalInitMethods.get(k)!
