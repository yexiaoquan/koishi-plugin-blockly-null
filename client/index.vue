<template>
  ...
  <k-layout class="page-blockly">
    ...
    <template #left>
      <SideBar :key="forceUpdate" :blocks="mainLocalBlocklyData.length > 0 ? mainLocalBlocklyData : store.blockly"
        v-model:current="currentId" :workspace="editor" :panel="blocklyToolboxInformation" :dialog="dialogOpenStates"
        :logger="build_console" :refresh-main="refreshMainData" />
    </template>
    <div style="display:flex;flex-flow:column nowrap;height: 100%;width: 100%">
      <div style="height: 100%; display: flex;flex-flow: column">
        ...
        <keep-alive
          v-show="(workspaceType === 'blockly' || workspaceType === 'meta') && (currentId != null && !loading) || init">
        ...
        <transition name="meta-ui-change">
          <div
            style="position: fixed;bottom: 0;top: 0;right: 0;width: 400px;background-color: #ffffff7a;z-index: 1000;backdrop-filter: blur(50px);box-shadow: 9px -7px 20px 0px black;background-size: 50px;"
            v-show="(workspaceType === 'meta') && (currentId != null && !loading) || init">
      ...
      <div class="transition-animation" style="border-top:3px solid var(--bg1);display:flex;flex-flow:column nowrap;"
        :style="{ height: currentPanelId.toString() === 'hidden' ? '25px' : '40%' }" v-if="currentId != undefined">
        <div style="height: 25px;background: var(--bg1);display: flex;width: 100%;z-index: 999;">
          <div
            style="height:100%;display: inline-flex;align-self: center;padding-left: 20px;padding-right: 20px;border-radius: 4px 4px 0px 0px;cursor: pointer;"
            @click="currentPanelId = 'build'" :style="{ background: currentPanelId == 'build' ? 'var(--bg3)' : '' }">编译</div>
          <div
            style="height:100%;display: inline-flex;align-self: center;padding-left: 20px;padding-right: 20px;border-radius: 4px 4px 0px 0px;cursor: pointer;"
            @click="currentPanelId = 'result'" :style="{ background: currentPanelId == 'result' ? 'var(--bg3)' : '' }">代码结果
          </div>
          <div
            style="height:100%;display: inline-flex;align-self: center;padding-left: 20px;padding-right: 20px;border-radius: 4px 4px 0px 0px;cursor: pointer;"
            @click="currentPanelId = 'log'" :style="{ background: currentPanelId == 'log' ? 'var(--bg3)' : '' }">运行日志</div>
          <div style="margin-left: auto;align-self: center;height: 100%;margin-right: 20px;">
            <div style="height: 18px;width: 18px;background: var(--bg1);padding: 2px;margin: 2px"
              @click="currentPanelId = 'hidden'">
              <window />
            </div>
          </div>
        </div>

        <div style="overflow:scroll;color:var(--fg2);height: 100%;width: 100%;contain: size" class="scroll"
          v-show="currentPanelId != 'hidden'" :style="{ background: currentPanelId === 'build' ? 'black' : 'var(--bg2)' }">
          <ToolboxBuild :current="currentId" ref="build_console" v-show="currentPanelId === 'build'" style="margin: 6px">
          </ToolboxBuild>
          <ToolboxCode :current="currentId" :blocklyInformation="blocklyToolboxInformation"
            v-show="currentPanelId === 'result'" v-if="currentPanelId != 'hidden'"></ToolboxCode>
        </div>
      </div>
    </div>
  </k-layout>
