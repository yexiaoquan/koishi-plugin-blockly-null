    ...
    <el-row>
      <el-col :span="16">
        <el-scrollbar height="200px" style="border: 1px lightgray solid;border-radius: 5px" class="editor-context">
          <div contenteditable="true" style="min-height: 200px;" class="editor-content" ref="inputModel"
            @click="updateSelection">
          </div>
        </el-scrollbar>
      </el-col>
      <el-col :span="8">
        <variable v-model="modelValue.variables" v-on:tag-click="insertVariable" v-on:tag-removed="tagRemoved">
        </variable>
      </el-col>
    </el-row>
  </el-dialog>
...
const dialogEnabled = computed({
  ...
  set: (val) => {
    // 在对话框关闭前，确保模板内容被更新
    if (!val && inputModel.value) {
      updateModel()
    }
    $emits('update:current', val ? 'text-template' : undefined)
  }
})
...
function updateModel() {
  ...
  const templateContent = text.join("").replace(/\n$/, "")
  modelValue.value.template = templateContent
  console.log("Template updated:", templateContent)
  updateSelection()
}
onMounted(() => {
