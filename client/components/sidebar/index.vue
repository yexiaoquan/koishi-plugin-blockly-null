...
<template>
  <div class="create"
    style="display: flex;flex-direction: row-reverse;padding-right: 10px;padding-top: 10px;height: 48px;">
    <el-popover placement="bottom" trigger="hover" content="新建插件">
      <template #reference>
        <i @click="createBlockly" style="cursor: pointer;padding-right: 20px;"><new-file /></i>
      </template>
    </el-popover>
    <el-popover placement="bottom" trigger="hover" content="导入代码">
      <template #reference>
  ...
  <div class="list" style="height: 60%">
    <el-scrollbar>
      <blockly-tab-group :key="forceUpdate"
        :data="Object.fromEntries((dataLoaded ? localBlocklyData : store.blockly).map(t => [t.id, t]))" v-model="current">
      </blockly-tab-group>
    </el-scrollbar>
  </div>
